...
function megaSena2454(){
    const resultabela = ""
    /* 2 - terça, 3 - Quarta, 4 - Quinta, 6 - Sábado  015	101415243444 */
    const resultadoMS =  insereNumeros(2452, "12/02/2022", "7.000.000,00", 10, 14, 15, 24, 34, 44, 3, premiosMegaSena = premiosMS (0, "0", 63, "35.542,46", "3.892", "821,89"), proxSorteio("16/02/2022", "12.000.000,00", 2454));
    
    const palpiteMSCorreto = palpiteProxSorteio("16/02/2022", "12.000.000,00", 2454);

    palpiteMSM.innerHTML = `${palpiteMSCorreto} ${resultadoMS}`;
    tConcurso.innerHTML = `${resultadoMS}`      
}

function megaSena2453(){
    const resultabela = ""
    /* 2 - terça, 3 - Quarta, 4 - Quinta, 6 - Sábado  015	101415243444 */
    const resultadoMS =  insereNumeros(2452, "12/02/2022", "7.000.000,00", 10, 14, 15, 24, 34, 44, 3, premiosMegaSena = premiosMS (0, "0", 63, "35.542,46", "3.892", "821,89"), proxSorteio("16/02/2022", "12.000.000,00", 2454));
...
function megaSena2452(){
    const resultabela = ""
    /* 2 - terça, 3 - Quarta, 4 - Quinta, 6 - Sábado  015	081051565758 */
    const resultadoMS =  insereNumeros(2452, "09/02/2022", "3.000.000,00", 08, 10, 51, 56, 57, 58, 3, premiosMegaSena = premiosMS (0, "0", 23, "72.470,70", "2.117", "1.124,78"), proxSorteio("12/02/2022", "7.000.000,00", 2453));
    
    const palpiteMSCorreto = palpiteProxSorteio("12/02/2022", "7.000.000,00", 2453);

    palpiteMSM.innerHTML = `${palpiteMSCorreto} ${resultadoMS}`;
    tConcurso.innerHTML = `${resultadoMS}`      
}
function megaSena2451(){
    const resultabela = ""
    /* 2 - terça, 3 - Quarta, 4 - Quinta, 6 - Sábado  015	132631465160 */
    const resultadoMS =  insereNumeros(2451, "05/02/2022", "26.000.000,00", 13, 26, 31, 46, 51, 60, 6, premiosMegaSena = premiosMS (1, "26.422.347,01", 60, "55.490,34", "4.536", "1.048,57"), proxSorteio("09/02/2022", "3.000.000,00", 2452));
    
    const palpiteMSCorreto = palpiteProxSorteio("09/02/2022", "3.000.000,00", 2452);

    palpiteMSM.innerHTML = `${palpiteMSCorreto} ${resultadoMS}`;
    tConcurso.innerHTML = `${resultadoMS}`      
}
function megaSena2450(){
    const resultabela = ""
    /* 2 - terça, 3 - Quarta, 4 - Quinta, 6 - Sábado  015	020611151739 */
    const resultadoMS =  insereNumeros(2450, "02/02/2022", "20.000.000,00", 02, 06, 11, 15, 17, 39, 3, premiosMegaSena = premiosMS (0, "0", 94, "26.814,82", "5.633", "639,24"), proxSorteio("05/02/2022", "26.000.000,00", 2451));
    
    const palpiteMSCorreto = palpiteProxSorteio("05/02/2022", "26.000.000,00", 2451);

    palpiteMSM.innerHTML = `${palpiteMSCorreto} ${resultadoMS}`;
    tConcurso.innerHTML = `${resultadoMS}`      
}

function megaSena2449(){
    const resultabela = ""
    /* 2 - terça, 3 - Quarta, 4 - Quinta, 6 - Sábado  015	142021314952 */
    const resultadoMS =  insereNumeros(2449, "29/01/2022", "36.000.000,00", 14, 20, 21, 31, 49, 52, 6, premiosMegaSena = premiosMS (1, "36.777.767,10", 65, "50.669,64", "3.771", "1.247,68"), proxSorteio("02/02/2022", "20.000.000,00", 2350));
    
    const palpiteMSCorreto = palpiteProxSorteio("02/02/2022", "20.000.000,00", 2350);

    palpiteMSM.innerHTML = `${palpiteMSCorreto} ${resultadoMS}`;
    tConcurso.innerHTML = `${resultadoMS}`  
    
}
function megaSena2448(){
    const resultabela = ""
    /* 2 - terça, 3 - Quarta, 4 - Quinta, 6 - Sábado  015	183032354048 */
    const resultadoMS =  insereNumeros(2448, "27/01/2022", "31.000.000,00", 18, 30, 32, 35, 40, 48, 4, premiosMegaSena = premiosMS (0, "0", 53, "51.387,47", "3.529", "1.102,51"), proxSorteio("29/01/2022", "36.000.000,00", 2449));
    
    const palpiteMSCorreto = palpiteProxSorteio("29/01/2022", "36.000.000,00", 2449);

    palpiteMSM.innerHTML = `${palpiteMSCorreto} ${resultadoMS}`;
    tConcurso.innerHTML = `${resultadoMS}`  
    
}
function megaSena2447(){
    const resultabela = ""
    /* 2 - terça, 3 - Quarta, 4 - Quinta, 6 - Sábado  015	131929424952 */
    const resultadoMS =  insereNumeros(2447, "25/01/2022", "28.500.000,00", 13, 19, 29, 42, 49, 52, 3, premiosMegaSena = premiosMS (0, "0", 42, "48.167,42", "3.080", "938,32"), proxSorteio("27/01/2022", "31.000.000,00", 2348));
    
    const palpiteMSCorreto = palpiteProxSorteio("27/01/2022", "31.000.000,00", 2348);

    palpiteMSM.innerHTML = `${palpiteMSCorreto} ${resultadoMS}`;
    tConcurso.innerHTML = `${resultadoMS}`  
    
}
function megaSena2446(){
    const resultabela = ""
    /* 2 - terça, 3 - Quarta, 4 - Quinta, 6 - Sábado  015	112532374756 */
    const resultadoMS =  insereNumeros(2446, "22/01/2022", "22.000.000,00", 1, 13, 27, 41, 51, 58, 6, premiosMegaSena = premiosMS (0, "0", 74, "41.305,51", "4.344", "1.005,19"), proxSorteio("25/01/2022", "28.500.000,00", 2347));
    
    const palpiteMSCorreto = palpiteProxSorteio("25/01/2022", "28.500.000,00", 2347);

    palpiteMSM.innerHTML = `${palpiteMSCorreto} ${resultadoMS}`;
    tConcurso.innerHTML = `${resultadoMS}`  
    
}
function megaSena2445(){
    const resultabela = ""
    /* 2 - terça, 3 - Quarta, 4 - Quinta, 6 - Sábado  015	112532374756 */
    const resultadoMS =  insereNumeros(2445, "19/01/2022", "16.000.000,00", 11, 25, 32, 37, 47, 56, 3, premiosMegaSena = premiosMS (0, "0", 63, "38.345,37", "3.802", "907,70"), proxSorteio("22/01/2022", "22.000.000,00", 2346));
    
    const palpiteMSCorreto = palpiteProxSorteio("22/01/2022", "22.000.000,00", 2346);
